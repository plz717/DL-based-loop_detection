...
def convert_to_array_queue(files, files_num, height, width, channels):
    ...
    # step 4: batching
    image_batch = tf.train.batch([gray_images], batch_sizre=1)
    batch_size = 1
...
DATA_SET_DIR = "/home/plz/slam/Images_12/"
files_num = 12  # number of train dataset
height = 480
width = 640
...
with tf.Session() as sess:
    ...
    for image in filenames:
        ...
        pad_im_test = tf.pad(
            im_test, [[0, 0], [amount, amount], [amount, amount], [0, 0]])

        encode_result = tf.nn.bias_add(tf.nn.conv2d(
            pad_im_test, W1, [1, stride, stride, 1], 'VALID'), b1)
        encode = tf.nn.tanh(encode_result)
        features_list.append(encode.eval())  # append an array to list
